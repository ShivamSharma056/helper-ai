...
async function generateContent() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      showStatus(status, data.error || 'Generation failed. Try again.', 'error');
      return;
    }
    ...
    loadDocuments();
  } catch(e) { showStatus(status, 'Network error. Check your connection and try again.', 'error'); }
}
...
function joinCollabRoom() {
  const roomId = document.getElementById('collab-room-name').value.trim();
  if (!roomId) return;
  socket.emit('join_room', { room: roomId });
  currentRoom = roomId;
